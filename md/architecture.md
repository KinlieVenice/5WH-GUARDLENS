...
1. **Tenant isolation, fail-closed.** Every query gets `tenantId` injected from request context; if context is missing, it **throws** rather than running unscoped. (§4.3)
2. **Transactional outbox.** State changes and their downstream effects are written in one DB transaction; a relay delivers them. No "save then send" that can half-fail. (§6)
3. **Append-only logs.** `AuditLog` and `LogbookEntry` are write-once; corrections are new rows. Server time is authoritative. (§4.9)
...
### 4.1 Authentication
- **Short-lived access token (~15 min)** + **refresh token** stored hashed in `Session`. Access token verified with no DB hit **[SCALE]**.
- **Refresh rotation:** each refresh swaps the token and kills the old; reuse of an old one ⇒ theft signal ⇒ revoke session.
- **Revocation:** set `Session.revokedAt`; the app logs out on next heartbeat.
...
- The **public guest form is unauthenticated** — no session; protected by rate-limit + CAPTCHA + the link's tenant scope.

*Plain terms: instead of one forever-token, hand out a 15-minute "day pass" plus a "renewal ticket." A stolen day pass dies fast; the renewal ticket is swapped each use, and reusing an old one means it was stolen → shut the session down.*

### 4.2 Authorization (RBAC + property scope)
- Per-route policy checks on `User.role`.
- A property-limited manager is checked against `UserPropertyAccess` **in addition to** the tenant check — two locks.
- **Default-deny:** a route with no policy is rejected.

*Plain terms: who you are (authentication) ≠ what you may do (authorization). A supervisor for 2 of 5 hotels sees only those 2 — the system checks "right company?" and "right property?" Anything not explicitly allowed is denied.*
...
- **Leak-test suite:** log in as Hotel A, assert zero rows of Hotel B — for every table and every risky op (findMany, createMany, nested write, aggregate, groupBy, upsert, raw). **A failure blocks the release.**

*Plain terms: the most important piece. Every query silently gets "...and only for this hotel." The critical choice is "fail closed": if the system ever can't tell which hotel, it refuses rather than guessing — because guessing wrong shows one hotel another's data. Automated tests try to break this; you don't ship if they get through.*
...
*Plain terms: caps how many tries per window, so bots can't hammer login or flood the anonymous form. Redis means the limit is shared across every server copy.*
...
- Uploads: type/size validated, **malware-scanned** before usable (mandatory for the guest form).
- Storage keys generated server-side; client filenames never trusted.

*Plain terms: evidence isn't at a guessable public URL. When an authorized person needs one, the system mints a link that works for minutes then dies, and records every touch — what makes it trustworthy in a claim or police request.*
...
*Plain terms: "real-time" = the control room sees a new alert instantly without refreshing. Socket.IO keeps a live wire to each screen, grouped into "rooms" by hotel/property so messages only reach the right room. If the wire is down, important stuff still arrives the slower way.*

---

## 6. Background work & the outbox (BullMQ)
Slow work always runs as a background job, never in the web request **[OPT][SCALE]**.
- **Outbox is the reliability backbone.** A state change writes a small "this happened" row **in the same transaction**. A relay worker reads those rows and fans them out (notifications, the logbook line, follow-on jobs).
- **Queues:** outbox relay, reports, notifications, analytics, ai, maintenance (purges).
- **Reliability:** jobs **idempotent** (safe to retry), limited retries + backoff + a dead-letter queue. Each job carries its tenant so isolation applies inside the worker.

*Plain terms: slow tasks (PDFs, emails) shouldn't make users wait, so workers handle them. The risk: if you save an incident then separately send its notification, a crash between loses it. The outbox saves "incident happened + notify" in the very same DB save as the incident — all-or-nothing — and a worker safely retries until it succeeds.*
...
*Plain terms: "send it" is split from "decide to send it." A provider outage just delays + retries, instead of losing the message or breaking the thing that triggered it.*
...
- **Deletion:** a first-class anonymize/tombstone op (`DeletionRequest`), not hard delete.
...
*Plain terms: the operational side of the privacy promises — show how long you keep data, prove you delete on schedule, honor a deletion request without breaking records, and hand a hotel a copy of their own data on request.*
